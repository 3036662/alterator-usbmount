...
int main(int argc, char *argv[]) {
  ...
  if (action=="add"){
    ...
    std::unique_ptr<udev_device, decltype(&udev_device_unref)> device(ptr_device, udev_device_unref);
    ...
    std::string dev_type;
    const char* p_devtipe=udev_device_get_property_value(device.get(), "DEVTYPE"); 
    if (p_devtipe !=NULL){
        dev_type=p_devtipe;
    }   
    //ID_PART_ENTRY_NUMBER
    int partitions=0;
    const char* p_partition_count=udev_device_get_property_value(device.get(), "ID_PART_ENTRY_NUMBER"); 
    ...
    chown(mount_point.c_str(), 500, 500);
    ...
    std::string opts="id=500,gid=500,umask=007";
    ulong flags=0;
    ...
    int res = mount(dev.c_str(), mount_point.c_str(), filesystem.c_str(),
                   flags, opts.c_str());
    if (res != 0){
